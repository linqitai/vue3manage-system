	...
	<div class="table">
		...
		<div class="container">
			...
			<el-table :data="tableData" border stripe class="table" ref="multipleTable" style="width: 100%">
				...
				<el-table-column prop="date" label="创建时间"  min-width="120"></el-table-column>
				<el-table-column prop="date" label="截止时间"  min-width="120"></el-table-column>
				<el-table-column prop="date" label="标题" show-overflow-tooltip min-width="200"></el-table-column>
				<el-table-column prop="date" label="创建人"  min-width="120"></el-table-column>
		...
		<el-dialog title="详情" v-dialogDrag :visible.sync="detailVisible" width="420px">
			<div class="voteTitle">{{voteData.title}}</div>
			<div class="voteDate clear">
				<div class="left">创建时间 {{voteData.createDate}}</div>
				<div class="right">截止时间 {{voteData.cutOffDate}}</div>
			</div>
			<div class="question clear" v-html="voteData.question"></div>
			<div class="questoinType">选择类型 {{voteData.questoinType}}</div>
			<div class="answer">
	...
	export default {
		...
		methods: {
			...
			handleEdit(index, row) {
				...
				// this.$router.push({path:'/operateGuide',query:{type:'edit'}});
				window.open('/#/operateGuide?type=edit','_blank')
